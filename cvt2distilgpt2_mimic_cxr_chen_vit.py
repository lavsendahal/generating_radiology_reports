...
class CvT2DistilGPT2MIMICXRChen(LightningModule):
    def __init__(
        ...
        logging.info(f"The hidden size of the loaded model is: {hidden_size}")

        #ViT Base is already at 768, so we don't need to project it

        self.encoder_projection = EncoderPermuteProject(
            permute_encoder_last_hidden_state=[0, 1, 2],
            encoder_last_hidden_state_size=768,
